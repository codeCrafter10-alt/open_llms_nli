...
def main(args):
    ...
    if args.eval:
        ...
        print(f"Dataset loading time: {dataset_load_time:.2f}s")
        ...
        with open("results.txt", "w") as f:
            f.write("Model: Bowman SNLI\n\n")
            f.write(f"Model test accuracy: {100*acc:.3f}%\n")
            f.write(f"Total test examples: {len(val)}\n")
            f.write(f"Correct predictions: {(all_preds == all_targets).sum()}\n\n")
            f.write(f"Model loading time: {model_load_time:.2f}s\n")
            f.write(f"Dataset loading time: {dataset_load_time:.2f}s\n")
